...
namespace UCSConsoleGameFramework.Scenarios
{
    internal class Shop : Scene
    {
        ...
        public override void ProcessOption(string playerOption)
        {
            ...
            int option;
            if (Int32.TryParse(playerOption, out option) && Items.Count >= option)    //verificando se a lista possui a opção escolhida ou possui mais 
            {
                option--;
                if (Player.Instance.Money >= Items[option].Value)
                {
                    Player.Instance.Backpack.Items.Add(new Item   //adicionando o produto comprado
                    {
                        Name = Items[option].Name,
                        Value = Items[option].Value
                    });

                    Player.Instance.Money -= Items[option].Value;   //reduzindo o money 
                }
                else
                {
            ...
            else
            {
                ...
                if (playerOption == "LISTAR")              //mostando os intens da mochila
                {
                    ...
                    Console.WriteLine("Pressione qualquer tecla pra continuar...");    //aquardando o usuario observar todos os itens
                    Console.ReadKey();
                }
